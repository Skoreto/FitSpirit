...
<c:choose>
	<c:when test="${user['new']}"><c:set var="currentTitle" value="Nová obsluha"/></c:when>
	<c:otherwise><c:set var="currentTitle" value="Úprava obsluhy"/></c:otherwise>
</c:choose>
...
<header class="page-heading clearfix">
    ...
    <div class="breadcrumbs pull-right">
		<ul class="breadcrumbs-list">
		 ...
		 <li><a href="<spring:url value="/instructors/index" htmlEscape="true"/>">Obsluha</a><i class="fa fa-angle-right"></i></li>
	    ...
	    <div class="col-sm-offset-2 col-sm-10">
	    	<c:choose>
	    		<c:when test="${user['new']}">
	    			<button type="submit" class="btn btn-default">Přidat obsluhu</button>
	    		</c:when>
	    		<c:otherwise>
           	 		<button type="submit" class="btn btn-default">Upravit obsluhu</button>
          		</c:otherwise>
	    	</c:choose>       
	    </div>
	</div>
